...
describe('Users Controller Test', () => {
    ...
    it('should return list of users', done => {
        request(app).get('/api/users').end((err: any, res: any) => {
    ...
    it('should not create a new user if name is not provided', done => {
        ...
        request(app).post('/api/users').send(user).end((err: any, res: any) => {
    ...
    it('should get information of user', done => {
        request(app).get('/api/users/1').end((err: any, res: any) => {
            let { data } = res.body;
    ...
    it('should respond with not found error if random user id is provided', done => {
        request(app).get('/api/users/1991').end((err: any, res: any) => {
    ...
    it('should update a user if name is provided', done => {
        ...
        request(app).put('/api/users/1').send(user).end((err: any, res: any) => {
            let { data } = res.body;
    ...
    it('should delete a user if valid id is provided', done => {
        request(app).delete('/api/users/1').end((err: any, res: any) => {
    ...
    it('should respond with not found error if random user id is provided for deletion', done => {
        request(app).delete('/api/users/1991').end((err: any, res: any) => {
            let { code, message } = res.body.error;
